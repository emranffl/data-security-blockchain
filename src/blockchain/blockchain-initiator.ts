...
import { PrismaClient, satellite_info_status, mobile_info_status, blockchain_type } from '@prisma/client'
...
const prisma = new PrismaClient(),
    initiateBlockchain = async () => {
        ...
        nodes.map(async (node, index) => {
            ...
            if (node.device_type == Device.Type.Satellite) {
            ...
            if (node.device_type == Device.Type.GroundStation) {
            ...
            if (node.device_type == Device.Type.PhasedArrayAntenna) {
            ...
            if (node.device_type == Device.Type.Mobile) {
            ...
            for (let status of Object.values(node.device_type != 'Mobile' ? satellite_info_status : mobile_info_status)) {
                ...
                nodeWallet.createORupdateLink({
                    ...
                    type: node.device_type.replace(/ /g, '_') as keyof typeof blockchain_type,
