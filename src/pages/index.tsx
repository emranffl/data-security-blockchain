...
export const getServerSideProps = async () => {
  let nodes: NodeDataSet[] = [], edges: EdgeDataSet[] = [],
    ...
    phasedArrayAntennaNodeLimit = randomInRange(MIN_PHASED_ARRAY_ANTENNA_NODE, MAX_PHASED_ARRAY_ANTENNA_NODE),
    ...
    [satelliteData, gs_data, phased_array_antenna_data, mobile_data] = await prisma.$transaction([
      ...
      prisma.phased_array_antenna_info.findMany({ take: phasedArrayAntennaNodeLimit, skip: randomInRange(0, 50) }),
  ...
  Object.values(ground_station_info_status).forEach((currentStatusInLoop) => {
    ...
    if (currentStatusInLoop == "Active") {
      ...
      for (let node of y.length > 0 ? y : []) { // implement y.len=0
        ...
        if (node.id != z.id && edges.some((edge) => { return edge.from != z.id }))
          edges.push({
            from: node.id,
            to: z.id,
            color: {
