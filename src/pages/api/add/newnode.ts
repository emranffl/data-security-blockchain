...
export default async function handler(
	...
	try {
		...
		if (deviceType == Device.Type.Satellite)
			if (
				!(await prisma.satellite_info.findUnique({
					where: { NORAD: deviceData.NORAD },
				}))
			) {
				// check if the connection request node is active
				return res.status(200).json({
					chain: await prisma.blockchain.findFirst({
						where: {
				...
				if (
					(
						await prisma.blockchain.findFirst({
							where: {
								uuid: connectToNode['device_id'],
							},
							orderBy: { transactionDate: 'desc' },
					...
					return res.status(400).json({
						name: 'NORAD',
						category: 'satellite_info',
						error: 'satellite status not active, refused to connect',
					})

				return res.status(200).json({
					data: await prisma.satellite_info.create({
						data: deviceData,
					}),
				})
			} else
				return res.status(400).json({
					name: 'NORAD',
					category: 'satellite_info',
					error: 'satellite with same NORAD already exists',
				})
		...
		if (deviceType == Device.Type.GroundStation) {
			...
			if (
				await prisma.ground_station_info.findUnique({
					where: { id: deviceData.id },
				})
			)
				return res.status(400).json({
					name: 'id',
					category: 'ground_station_info',
					error: 'ground station with same ID already exists',
				})
...
async function createNodeAndMineBlock(nodeData: any, connectToDevice: any) {
	...
	const nodeWallet = new Wallet(),
		...
		// mine & add the block
		newBlock = await nodeWallet.createORupdateLink(
			nodeData,
